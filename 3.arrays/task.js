...
function getUsersNamesInAgeRange(users, gender) {    
  const usersSorted = users.filter(user => user.gender === gender);
    if(usersSorted.length === 0) {
      return 0
    }
  const sortedAges = usersSorted.map(person => person.age);
  const average = sortedAges.reduce((acc, item) => { return acc + item; }, 0) / sortedAges.length;
  return average
}
